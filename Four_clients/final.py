...
while len(all_lines) < tot+1:
    ...
    xx = yy
    yy = len(all_lines)
    if xx//50 != yy//50 and yy<1000:
        if 50*(yy//50) not in a:
            a.add(len(hash))
            print("time taken for ", 50*(yy//50),"lines is ", time.time()-start)
    # print("inside loop")
    for z in range (0, 1):
        vayu.send("SENDLINE\n".encode("utf-8"))
        data = ""
...
print("time taken for 1000 lines is  ", time.time()-start)
